...
import { X, Sparkles, Hash, TrendingUp, Camera, Upload, StickyNote, AlertTriangle, Plus, Paperclip, Send, Reply, FileText, Image as ImageIcon, Video, FileSpreadsheet, Bold, Italic, List, ListOrdered, MoreHorizontal, Edit, Trash2, Link, Smile, Menu, Play, Calendar, Clock, MessageSquare, MoreVertical, Code, AlignLeft, AlignCenter, AlignRight, Type, Eye, Monitor, Smartphone, Heart, MessageCircle, Bookmark, ThumbsUp, Share, Repeat2, ChevronLeft, ChevronRight, Download, ArrowLeft, Share2 } from "lucide-react";
      ...
      <div className={cn(
        ...
        {isDesktop ? (
              ...
              <div className="flex-1 overflow-y-auto p-4 bg-gray-50">
                {selectedPlatforms.length > 0 ? (
                  (() => {
                    ...
                    if (!platform) return <div className="text-gray-500 text-center">Select a platform to see preview</div>;
                        ...
                        {selectedPreviewPlatform === 'instagram' && (
                          <div className="bg-white rounded-lg p-4 shadow-sm max-w-sm mx-auto">
                            <div className="flex items-center gap-3 mb-3">
                              ...
                              <div>
                                <div className="font-semibold text-sm">{selectedBrand?.name || 'Brand Name'}</div>
                            ...
                            <div className="text-sm">
                              <span className="font-semibold">{selectedBrand?.name || 'brand'}</span>
                        ...
                        {selectedPreviewPlatform === 'facebook' && (
                          <div className="bg-white rounded-lg p-4 shadow-sm">
                            <div className="flex items-center gap-3 mb-3">
                              ...
                              <div>
                                <div className="font-semibold">{selectedBrand?.name || 'Brand Name'}</div>
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                  <span className="font-bold">{selectedBrand?.name || 'Brand Name'}</span>
                                  <span className="text-gray-500">@{selectedBrand?.name?.toLowerCase().replace(/\s+/g, '') || 'brand'}</span>
                          ...
                          <div className="bg-black rounded-lg p-4 text-white shadow-sm">
                            <div className="flex items-center justify-between mb-4">
                              <div className="flex items-center gap-3">
                                ...
                                <span className="font-semibold">{selectedBrand?.name || 'Brand Name'}</span>
                              </div>
                            </div>
          ...
          <div className="flex-1 flex flex-col">
            ...
            {/* Mobile Content */}
            <div className="flex-1 overflow-y-auto">
              {/* Mobile content would go here - keeping existing mobile logic */}
            </div>
          </div>
        )}

        {/* Sliding Menu - overlays on preview panel with transparency */}
        <div className={cn(
          ...
          <div className="flex-1 p-6 overflow-y-auto">
            <div className="space-y-6">
              ...
              {/* Preview Settings */}
              <div>
                <h4 className="font-medium mb-3">Preview Settings</h4>
                <div className="space-y-2">
                  {platforms
                    .filter(platform => selectedPlatforms.includes(platform.id))
                    .map((platform) => {
                      ...
                      return (
                        <Button
                          key={platform.id}
                          variant={selectedPreviewPlatform === platform.id ? "default" : "outline"}
                          className="w-full justify-start"
                          onClick={() => setSelectedPreviewPlatform(platform.id)}
                        >
